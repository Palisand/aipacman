...
public final class Game
{
    ...
    public MOVE[] myGetPossibleMoves(int nodeIndex){
        MOVE[] moves = new MOVE[currentMaze.graph[nodeIndex].neighbourhood.entrySet().size()];
        int i = 0;
        for( Entry<MOVE, Integer> entry : currentMaze.graph[nodeIndex].neighbourhood.entrySet()){
    ...
    public MOVE breadthFirstSearchMove( Game game, Controller<EnumMap<GHOST, MOVE>> ghostController )
    {
        MOVE nextMove = MOVE.NEUTRAL;
        LinkedList<Game> frontier = new LinkedList<Game>();
        HashMap<Integer, Stack<MOVE>> paths = new HashMap<Integer, Stack<MOVE>>();
        ...
        while( !frontier.isEmpty() && !routeFound ){
            Game currentGame = frontier.pop();

            if(!paths.containsKey(currentGame.getPacmanCurrentNodeIndex())){
                paths.put(currentGame.getPacmanCurrentNodeIndex(), new Stack<MOVE>());
            }
            Stack<MOVE> pathSoFar = paths.get(currentGame.getPacmanCurrentNodeIndex());

            if(!explored.contains(currentGame.getPacmanCurrentNodeIndex())) {
                for(MOVE move : currentGame.myGetPossibleMoves(currentGame.getPacmanCurrentNodeIndex())){
                    ...
                    if(!paths.containsKey(copy.getPacmanCurrentNodeIndex())){
                        paths.put(copy.getPacmanCurrentNodeIndex(), (Stack<MOVE>) pathSoFar.clone());
                    }
                    paths.get(copy.getPacmanCurrentNodeIndex()).push(move);
                    frontier.add(copy);
                    int currentScore = copy.getScore();
                    if ((currentScore > maxScore)) {
                        while(!paths.get(copy.getPacmanCurrentNodeIndex()).isEmpty()){
                            nextMove = paths.get(copy.getPacmanCurrentNodeIndex()).pop();
                        }
                        routeFound = true;
                        break;
                    }
    ...
    public MOVE getNextMoveTowardsTargetDFS(int fromNodeIndex, int toNodeIndex, DM distanceMeasure)
    {
        MOVE move = null;
        double minDistance=Integer.MAX_VALUE;
        Stack<Entry<MOVE, Integer>> stack = new Stack<Entry<MOVE, Integer>>();
        Entry<MOVE, Integer> fromNodeEntry = new AbstractMap.SimpleEntry<MOVE, Integer>(null, fromNodeIndex);
        Set<Integer> visited = new HashSet<Integer>();  // empty, none visited
